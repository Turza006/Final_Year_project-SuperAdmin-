...
export default {
  ...
  watch: {
    selectedCategoryId (newValue) {
      ...
      this.getAllServiceByCategoryIdNetworkCall(newValue)
    }
  },
  async created () {
    this.loading = true
    ...
    this.loading = false
    this.$nuxt.$on(DIALOG_EVENT.TOGGLE_EDIT_PACKAGE_DIALOG, (data) => {
      this.packageId = data.id
      this.form.packageName = data.packageName
      this.selectedCategoryId = data.ServiceCategory.id
      this.form.ServiceCategory = data.ServiceCategory.id
      this.form.serviceCovered = []
      this.selectedCategoryId = this.form.ServiceCategory
      data.serviceCovered.forEach((service) => {
        this.form.serviceCovered.push(service.id)
      })
  ...
  methods: {
    ...
    ...mapActions('package', ['requestEditPackage']),
    async getAllServiceByCategoryIdNetworkCall (CategoryId) {
    ...
    async submitForm () {
      if (this.$refs.form.validate()) {
        ...
        const res = await this.requestEditPackage({
          $apollo: this.$apollo,
          payload: this.form,
