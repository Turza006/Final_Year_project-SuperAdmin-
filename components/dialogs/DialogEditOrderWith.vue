      ...
      <v-form ref="form" class="pa-10">
        <v-layout>
          <v-flex xs12 md6 sm6>
            ...
            <ImageInput class="my-4" :image-link="form.logo" @changeImagelink="updateImageUrl($event)"/>
          </v-flex>
        </v-layout>
        <label>Name *</label>
        <v-text-field
...
export default {
  ...
  created () {
    this.$nuxt.$on(DIALOG_EVENT.TOGGLE_EDIT_ORDER_WITH, (orderWith) => {
      this.orderWithId = orderWith.id
      this.form.name = orderWith.name
      this.form.logo = orderWith.logo
      this.form.textColour = orderWith.textColour
      this.form.FillColour = orderWith.FillColour
      this.form.customColour = orderWith.customColour
      this.dialog = !this.dialog
    })
  },
  methods: {
    ...mapActions('orderWith', ['requestEditOrderWith']),
    updateImageUrl (url) {
      this.form.logo = url
    },
    async submitForm () {
      ...
      if (this.$refs.form.validate()) {
        this.loading = true
        const res = await this.requestEditOrderWith({
          $apollo: this.$apollo,
          payload: this.form,
          OrderWithId: this.orderWithId,
